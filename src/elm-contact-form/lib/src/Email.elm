...
decoder : Decoder String Problem Email
decoder =
    Decoder.custom <|
        \input ->
...
type Problem
    ...
    | EmptyHostName -- "empty host name is not allowed"
    | InvalidSubdomain -- "domain part must conatin at least one subdomain"
    | InvalidTld -- "tld must be in alphabet"
    | TooLongEmail -- "the total length must be less than 254 characters"
    | MissingHostName -- "domain name must be followed by a top level domain"
